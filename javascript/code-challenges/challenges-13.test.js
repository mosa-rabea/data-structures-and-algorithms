...
const longestString = (arr) => {
  const stingLon = arr.reduce((a,b)=>{
    return a = b.length > a.length ? b : a;
  },'');
  return arr.indexOf(stingLon);
};
...
const firstLetters = (arr) => {
  let newArray = arr.map(each => each[0]);
  return newArray;
};
...
const findHappiness = (arr) => arr.filter(each => each.includes(':)'));
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(each => each.replace(/[() -]/g, ''));
};
...
const onlyOddChars = (b) => {
  return b.split('').filter((value,index)=>index%2 === 1).join('');};
...
const allHappy = (arr) => {
  return arr.every((each)=>each.includes(':)'));
};
