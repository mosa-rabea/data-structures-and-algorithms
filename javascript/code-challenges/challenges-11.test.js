...
function transformToLis(obj){
  return Object.entries(obj).map((each)=>`<li>${each.join(': ')}</li>`);
}
...
const totalSum = (input) => {
  let total = input.reduce((acc,value )=> {
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(newArray => newArray.filter(a => (typeof a === 'number' && a % 5 === 0)).map(b => Math.pow(2, b)));};
...
let findMaleAndFemale = (data) => {
  return data.filter(a => a.gender === 'male' || a.gender=== 'female').map(b => b.name).join(' and ');
};
