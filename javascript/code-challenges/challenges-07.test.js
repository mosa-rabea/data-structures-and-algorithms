...
const sortStarWarsCharacters = (arr) =>
  arr.sort((x, y) => (+y.height > +x.height ? 1 : -1));
...
const howMuchPencil = (str) => {
  ...
  let result = [];
  let strarr = [...str,' '];
  strarr.forEach((each,index) => { result.push( str.slice(index,strarr.length)); });
  return result;
};
...
returns a new array where every each is a character of the input string.
...
const listFoods = (recipe) => {
  ...
  for (const i in [...recipe.ingredients]) {
    const each =   (recipe.ingredients[i].slice(
      recipe.ingredients[i].indexOf(' ') + 1,
      recipe.ingredients[i].length ));
    result.push(each.slice(each.indexOf(' ') + 1,each.length) );
  }
...
Write a function named extractVowels that takes in a string and returns an array where the first each is the original string with all the vowels removed, and the second each is a string of all the vowels that were removed, in alphabetical order.
